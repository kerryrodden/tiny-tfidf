...
export default class Corpus {
  ...
  // Collection frequency weight (a.k.a. inverse document frequency). Compared to the formula in
  // the original paper, we add 1 to N (the number of documents in the collection) so that terms
  // which appear in every document (and are not stopwords) get a very small CFW instead of zero
  // (and therefore, later, get a very small weight instead of zero, meaning that they can still
  // be retrieved by queries and appear in similarity calculations).
  ...
  // and scores. The score for a document is the total combined weight of each query term that
